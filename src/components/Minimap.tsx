...
/**
 * The node rectangles, split out and memoised.
 *
 * Only `pan` and `zoom` change while the canvas is being panned, and neither affects a
 * single one of these rects — but they live in the same component as the viewport frame,
 * so all 109 of them were rebuilt and reconciled on every frame of every pan. Everything
 * this needs derives from `nodes` and `sizes`, so the memo genuinely bails out and the
 * frame is left redrawing one rectangle.
 *
 * It has to be a child component rather than a useMemo, because Minimap returns early
 * when there is nothing to draw and a hook cannot live below that.
 */
    ...
    <>
      {nodes.map((n) => {
        ...
        return (
          <rect
            key={n.id}
            x={offX + (n.x - worldMinX) * scale}
            y={offY + (n.y - worldMinY) * scale}
